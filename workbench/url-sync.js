...
// activePageId 单向镜像到 ?page=&mode=（mode 是从页派生的只读形态，2026-08-16
// 阶段 2）：replaceState 只替换不堆历史
// （所以无需 popstate 处理），每次切换后地址栏即可直接复制当深链用。
// 读取侧（boot 时 URL 优先于 prefs）在 stage.js resolveBootPageId。
...
export function startDeepLinkSync() {
  var last = null;
  function sync(state) {
    var query = deepLinkQuery(state.activePageId, modeForPage(state.pageManifest, state.activePageId));
