 ...
 * 不存在的账本：文档文件删了、url 条目移走了、挂靠条目改挂别页了。
 * 这里只做判定与计数，不动盘；清理只经 `ppnt prune`。
 *
 ...
 * 页本身不在 registry 与 manifest 里 = 整桶皆孤儿（CLI 侧按桶判）。
 */
...
/** 一本账本是否孤儿。name = 桶内文件名；doc = 解析后的账本；spaces =
    pageSurfaceSpaces 的返回。保守取向：path 缺失（存量）或落在未知 URL 空间
    （错放，不是表面消失）都不判孤儿。 */
