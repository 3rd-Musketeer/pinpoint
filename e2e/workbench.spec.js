...
// gutter 事件驱动故事（1010 / 1050 / 1121 合并，2026-09-24 e2e 审计）：共用
// openSidebarGutter 前序，串成一条顺序故事走完四个触发源 —— 静止零渲染、
// 滚动增量重排、筛选 / 账本变化、晚到图片 load。滚动段会把 h1 滚出 iframe
// 视口（气泡摘除），每段之间滚回顶部等气泡回来再量。1083（跨页拆挂）要换
// iframe 实例，单独留一条。
test('gutter renders only on events: idle, scroll, filter/ledger changes, and late layout shifts', async ({ page }) => {
  await openSidebarGutter(page);

  // 滚回文档顶部，等离屏摘掉的气泡（h1）重新渲染出来。
  const backToTop = async () => {
    ...
    await expect.poll(() => page.evaluate(() => {
      ...
      return bubbles.filter((b) => /评论 h1|评论 #s2/.test(b.textContent || '')).length;
    })).toBe(2);
  };

  await test.step('gutter renders only on events: idle costs nothing, scroll repositions without rebuilding', async () => {
  ...
  await test.step('gutter tracks filter switches and new annotations through the snapshot subscription', async () => {
  ...
  await test.step('gutter follows late layout shifts: an image that gets its size after the page settles', async () => {
    await waitGutterQuiet(page);
...
test('sheet captions, outline tree, and right annotation panel (2026-08-15 侧栏重构)', async ({ page }) => {
  ...
  await expect(page.locator('#wbann-status')).toHaveAttribute('title', '待验收 1 条，共 1 条');
...
// 钉子与评论卡故事（2322 / 2380 合并，2026-09-24 e2e 审计）：先断钉子样式与
// 常显、评论卡 hover 才出，再用状态端点给第一条挂 note 走 SSE 回流，验折叠头
// 交互；第二条全程无 note 作对照。2322 的「overlay z > dock」与层级故事里的
// 命中断言重复，随合并删。
test('画布钉子常显高对比，评论卡 hover 钉子才出；agent 备注折叠头默认收起', async ({ page }) => {
  await openWorkbench(page);
  await page.evaluate(() => window.pinpoint.clear());
  await expect.poll(() => page.evaluate(() => window.pinpoint.marks.length)).toBe(0);
  await page.evaluate(() => window.pinpoint.setMode(true));
  ...
  await test.step('画布钉子常显高对比，评论卡 hover 钉子才出（2026-09-04 H owner 批注 1）', async () => {
    await expect(badges).toHaveCount(2);

    // 钉子：22px accent 实心圆 + 白字 + 白描边（高对比，常显不打折）
    const pin = await badges.nth(0).evaluate((el) => {
    ...
    await expect(bubble1).toHaveCount(1);
  ...
  await test.step('画布评论卡的 agent 备注：折叠头默认收起，点开看全文再收起；无 note 的卡没有折叠头', async () => {
    // note 走状态端点（与 ppnt mark 同一条路），SSE 回流后画布账本带上；
    // 卡正文下出折叠头，展开 / 收起只改页面内记忆。
