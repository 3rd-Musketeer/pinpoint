...
test('混合板：静态形态、文档/画布切换、条目跨 reload 记忆与深链', async ({ page }) => {
  await openMixed(page);

  // ── 静态形态：画布上只有 app/lock 屏；doc 屏在场但被条目显隐收起，绝不进画布。
  ...
  // ── 阅读器切换：选中产物文档条目 → doc iframe 1:1 铺满，画布 chrome 退场，树收起。
  ...
  // ── 条目记忆：选中态落在 prefs.activeEntryIdByPage，reload 直接回到草稿阅读器。
  ...
  // ── 深链：?entry= 直达文档条目；未知条目 id → 选中解析落默认条目（画布），
  //     URL 被重写为真实值。
