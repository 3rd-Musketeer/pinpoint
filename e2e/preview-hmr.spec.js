...
// 两屏都重编都更新。「只重编该屏」用 dist 产物 mtime 观察：没被点名的屏
// 产物不该被重写。
...
function registerHmrSite() {
  const doc = JSON.parse(fs.readFileSync(E2E_REGISTRY, 'utf8'));
  doc.entries.push({ id: 'e2e-hmr', title: 'E2E HMR', kind: 'dir', path: SITE_DIR, board: 'ios' });
...
const distFile = (name) => path.join(DIST_ENTRY, name);
...
test('改一屏：只重编该屏，画板收到 preview:update 后出新内容', async ({ page }) => {
  ...
  const betaBefore = fs.statSync(distFile('beta.html')).mtimeMs;
  fs.writeFileSync(path.join(SITE_DIR, 'alpha.jsx'), ALPHA_V2);
  // preview:update → 画板重摆：不等手刷就看到 v2。
  await expect(alpha).toContainText('hmr-site alpha v2');
  await expect(beta).toContainText('hmr-site beta v1');
  // 「只重编该屏」的观察点：没点名的 beta 产物一个字节没动。
  expect(fs.statSync(distFile('beta.html')).mtimeMs).toBe(betaBefore);
  expect(fs.statSync(distFile('alpha.html')).mtimeMs).toBeGreaterThan(betaBefore);
});

test('改共享组件：两屏都重编、画板两帧都更新', async ({ page }) => {
  test.setTimeout(60_000);
  await openBoard(page);
  const alpha = page.locator('#wb-board-panel [data-screen="alpha"]');
  const beta = page.locator('#wb-board-panel [data-screen="beta"]');
  await expect(alpha).toContainText('chip v1');
  await expect(beta).toContainText('chip v1');

  const alphaBefore = fs.statSync(distFile('alpha.html')).mtimeMs;
  const betaBefore = fs.statSync(distFile('beta.html')).mtimeMs;
  fs.writeFileSync(path.join(SITE_DIR, 'components', 'Chip.jsx'), CHIP_V2);
  // 共享组件在两屏的 deps 里：两屏都进重编名单，preview:update 后两帧都出新。
  await expect(alpha).toContainText('chip v1·改');
  await expect(beta).toContainText('chip v1·改');
  expect(fs.statSync(distFile('alpha.html')).mtimeMs).toBeGreaterThan(alphaBefore);
  expect(fs.statSync(distFile('beta.html')).mtimeMs).toBeGreaterThan(betaBefore);
});
