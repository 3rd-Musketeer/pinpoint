...
export function createAnnotationStore(options) {
  ...
  function setReply(input) {
    ...
    const index = disk.annotations.findIndex((annotation) => annotation && annotation.id === annotationId);
    if (index < 0) {
      return { status: 404, error: 'annotation_not_found', doc: disk };
    }

    const annotations = disk.annotations.map((annotation) => ({ ...annotation }));
    const content = typeof input.reply === 'string'
      ? input.reply
      : input.reply && input.reply.content;
    const author = input.reply && input.reply.author;
    const reply = normalizeAnnotationReply({
      content,
      author,
      updated_at: now().toISOString(),
    });
    if (reply) annotations[index].reply = reply;
    else delete annotations[index].reply;
