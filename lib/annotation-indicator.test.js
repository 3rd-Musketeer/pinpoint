...
test('normalizes the lightweight annotation reply without creating a thread model', () => {
  assert.deepEqual(normalizeAnnotationReply('  已按 frame 对齐  '), {
    content: '已按 frame 对齐',
    author: 'agent',
  });
  assert.deepEqual(normalizeAnnotationReply({
    content: '我补充一个细节',
    author: 'user',
  ...
  assert.equal(normalizeAnnotationReply({ content: '  ' }), undefined);
  ...
  assert.deepEqual(annotation.reply, { content: '已修改', author: 'agent' });
});
