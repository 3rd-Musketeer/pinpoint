...
test('setReply updates only one annotation with revision checks', (t) => {
  const { store } = withStore(t);
  const initial = store.save({
    page: 'index.html',
    baseRevision: 0,
    annotations: [
      { n: 1, id: 'ab12cd', content: '调整间距' },
      { n: 2, id: 'ef34gh', content: '保留原样' },
    ],
  });
  assert.equal(initial.status, 200);

  const updated = store.setReply({
    page: 'index.html',
    annotationId: 'ab12cd',
    baseRevision: 1,
    reply: { content: ' 已改为 16pt ', author: 'agent' },
  });
  assert.equal(updated.status, 200);
  assert.equal(updated.doc.revision, 2);
  assert.deepEqual(updated.doc.annotations[0].reply, {
    content: '已改为 16pt',
    author: 'agent',
    updated_at: '2026-07-10T00:00:00.000Z',
  });
  assert.deepEqual(updated.doc.annotations[1], { n: 2, id: 'ef34gh', content: '保留原样' });

  assert.equal(store.setReply({
    page: 'index.html',
    annotationId: 'ab12cd',
    baseRevision: 1,
    reply: { content: 'stale', author: 'agent' },
  }).status, 409);
  assert.equal(store.setReply({
    page: 'index.html',
    annotationId: 'missing',
    baseRevision: 2,
    reply: { content: 'missing', author: 'agent' },
  }).status, 404);

  const removed = store.setReply({
    page: 'index.html',
    annotationId: 'ab12cd',
    baseRevision: 2,
    reply: { content: '', author: 'agent' },
  });
  assert.equal(removed.status, 200);
  assert.equal(removed.doc.annotations[0].reply, undefined);
});
