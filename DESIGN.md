...
- 比例读法：zoom 50% 写作 1:2，HUD 按图纸比例显示。

## 侧栏分工（已拍定，已落地）

- 左栏 = 页面上下文：Pages（导航）→ 大纲（当前页 section → frame 树：引用号 + 屏名 + 计数徽标（红 = 含失效锚点）；点击定位 frame，与标注卡焦点双向同步；层次 = 延伸线结构：14px rail 槽位、字母与导线同轴、逐行拼接、末行收 └ 角，decisions 08-15c）→ 设置（齿轮进设置视图）。更改栏与铭牌是 REV 机制的展示面，随 REV 缓期（见下），不进第一批。宽度 200–480 拖拽（默认 250），双击 splitter 折叠。
