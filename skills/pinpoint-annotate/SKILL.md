...
owner 常用口头说法指页（“routines 原型”“那个雾玻璃界面”），不给页 id。先跑
`ppnt list <关键词>`：按 id、标题、源路径、文件夹宽松匹配（英文复数自动去 s），每页给
标题、类型、open / check 计数和源路径。只匹配到一页就用它；匹配到多页或一页都没有，
把候选列给 owner 问，不要猜。其他命令找不到页时也会列出最接近的候选。

## 1. `ppnt check <页>` 读

默认列出 open 状态、按帧分组：序号、帧号、正文、意图、源码摘录。`--frame B3` 收窄到一帧，
`--group-by component` 按组件分组（组头给共用帧数），`--status all` 看 check / done / close，
`--json` 给结构化输出。标注住该页的桶 `~/.pinpoint/<页>/`（画布 + mention 帧是 `@canvas.json`，
文档按路径分本），`#n` 只在本页桶里解析。清单末尾可能出现「孤儿标注」组：表面已不存在的
账本（文件删了 / 页没了），不计入计数、不要去 mark 它们 —— 清理归 owner 走 `ppnt prune`。
只读，无副作用；不要为读标注重启服务。
