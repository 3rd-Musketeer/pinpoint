...
- **存量视口一次性迁移**：pageViewports 的 canvasZoom 全部 ×2（clamp 到新轴范围）保持每页视觉不变，`zoomAxis:2` 标记防重跑（仿 migrateLegacyCanvasZoom 先例；`data-fit` 自动适配与画布无关，preview-mount 挂板时剥除，不受影响）。**2026-09-24 删除**：迁移期已过，存量存档均已迁完，`migrateZoomAxis2` 连同标记与 e2e 用例一并撤下。
