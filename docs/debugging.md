...
防回归：哪条测试/规则挡着（没有就写“无”，并考虑补；能在单测挡住就补单测，见 AGENTS.md“回归测试先写单测”）
```
...
防回归：`e2e/section-scroll.spec.js` 在 75%、117% 下通过真实 UI 创建标注、点击定位、返回上方并继续新增标注。修复前正是这两档失败，内部 scrollTop 非零；修复后要求偏移为零、上方 frame 可见可命中、原标注仍保留。（250% 档修复前也过，是阴性对照，2026-09-24 审计退役。）
